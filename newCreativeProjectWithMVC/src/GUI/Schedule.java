...
public class Schedule {
	...
	private LinkedList<LinkedList<AdjacencyTreeNode>> _choicedTimeTables;	//사용가능한 시간표 리스트
	private ArrayList<Object[][]> _arrayForWebShow;
	
	private void setArrayForWebShow(ArrayList<Object[][]> newList) {
		this._arrayForWebShow = newList;
	}
	
	public ArrayList<Object[][]> arrayForWebShow(){
		return this._arrayForWebShow;
	}
	...
	public Schedule(LinkedList<LinkedList<AdjacencyTreeNode>> aList) { // 기본적인 초기화
		...
		this.setArrayForWebShow(new ArrayList<Object[][]>(aList.size()));
	}
			...
			while (sIterator.hasNext()) {
				...
				if (realClass.time()[0][0] != -1) { // 요일 지정
					...
					for (int i = (int) ((realClass.time()[0][1] - 9) * 2 + 1); i < (int) ((realClass.time()[0][2] - 9)
							* 2 + 1); i++) {
						schedule[FirstDay][i] = (Object) (realClass.subject() + "/" + realClass.professor());
					}
				}
				if (realClass.time()[1][0] != -1) { // 요일 지정
					...
					for (int i = (int) ((realClass.time()[1][1] - 9) * 2 + 1); i < (int) ((realClass.time()[1][2] - 9)
							* 2 + 1); i++) {
						schedule[SecondDay][i] = (Object) (realClass.subject() + "/" + realClass.professor());
					}
				}
			}
			this.arrayForWebShow().add(schedule);
			this.objArray().add(this.reverseTheTalbe(schedule, 6, 28)); // Array에 추가
		}
	}

	private void makeArrayEmpty(Object[][] schedule, int wid, int length) {	//배열 초기화
		for (int i = 0; i < wid; i++) {
	...
	private Object[][] reverseTheTalbe(Object[][] obj, int wid, int length) {	//배열 점대칭 부분
