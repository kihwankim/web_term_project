...
public class dataBaseList {
	...
	private LinkedList<TreeNode> allSubjectsList(String str) {//하나의 과목으로 여러개의 
		String[] pagesData = this.makeDocumentstoString(str);
		LinkedList<TreeNode> newList = new LinkedList<TreeNode>();

		String[][] makeList;
		makeList = new String[pagesData.length][];//page단위의 string 배열 형성

		for (int i = 0; i < pagesData.length - 1; i++) {//web page의 객체 단위로 나누기
			String[] temp = pagesData[i].split("\\{");
			makeList[i] = new String[temp.length - 2];
			System.arraycopy(temp, 2, makeList[i], 0, temp.length - 2);//복사해서 넣기
		}

		for (int i = 0; i < makeList.length - 1; i++) {//해당 객체에서 다시 node로 형성
			for (int j = 0; j < makeList[i].length; j++) {
				TreeNode addedNode = new TreeNode(makeList[i][j]);//각 분반에 넣어준다
