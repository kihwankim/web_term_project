...
public class AppController {
	protected dataBaseList _databaseList; // 컴퓨터공학과 전공만을 위한 저장 공간
	protected dataBaseList _liberatorDataBaseList; // 교양과목에대한 정보 저장 공간
	protected MakeTheNodeList<AdjacencyTreeNode> _makeTheNodeList; // 필요한 과목의 정보만 가져오는 list형태 객체
	protected Choice _choice; // 과목, 교수님 명을 입력받고 예외처리한 것을 저장하는 객체
	protected ChoiceOfExtraStuffs _ChoiceOfExtraStuffs; // 교양을 추가 받는 것을 판별 및 공강, 시간표 출력 하때 선택
	protected TimeTableTree<AdjacencyTreeNode> _timeTableTree; // 실제 시간표를 작성하는 객체
	protected LinkedList<AdjacencyTreeNode[]> _forTreeList; // 컴퓨터 공학과 전공만을 위한 저장장 공간에서 모든 전공을 저장하는 list
	protected Schedule _schedule; // 시간표를 저장하는 객체
	protected Gui _gui; // gui->시간표 시각화
	protected Recomand<AdjacencyTreeNode> _recomand; // 공강을 추천 해 주는 객체

	protected void setLiberatorDataBaseList(dataBaseList aList) {
		this._liberatorDataBaseList = aList;
	}

	protected dataBaseList liberatorDataBaseList() {
		return this._liberatorDataBaseList;
	}

	protected void setRecomand(Recomand<AdjacencyTreeNode> newRec) {
		this._recomand = newRec;
	}

	protected Recomand<AdjacencyTreeNode> recomand() {
		return this._recomand;
	}

	protected void setGui(Gui newGui) {
		this._gui = newGui;
	}
	...
	protected void setChoiceOfExtraStuffs(ChoiceOfExtraStuffs newChoice) {
		this._ChoiceOfExtraStuffs = newChoice;
	}

	protected ChoiceOfExtraStuffs ChoiceOfExtraStuffs() {
		return this._ChoiceOfExtraStuffs;
	}

	protected void setSchedule(Schedule newSchedule) {
	...
	protected void setDataBaseList(dataBaseList newbase) {
	...
	protected void setMakeTheNodeList(MakeTheNodeList<AdjacencyTreeNode> newList) {
		this._makeTheNodeList = newList;
	}

	protected MakeTheNodeList<AdjacencyTreeNode> makeTheNodeList() {
		return this._makeTheNodeList;
	}

	protected void setChoice(Choice newChoice) {
	...
	protected void setTimeTableTree(TimeTableTree<AdjacencyTreeNode> newTimeTableTree) {
		this._timeTableTree = newTimeTableTree;
	}

	protected TimeTableTree<AdjacencyTreeNode> timeTableTree() {
		return this._timeTableTree;
	}

	protected void setLinkedList(LinkedList<AdjacencyTreeNode[]> newForTreeList) { // database손상을 막기 위해서 만든 임의의 list
		this._forTreeList = newForTreeList;
	}

	protected LinkedList<AdjacencyTreeNode[]> forTreeList() {
		return this._forTreeList;
	}

	protected String yORn() { // y 와 n를 입력 받음 -> 교양 과목 추천 받을 때 3개씩만 출력된다 이때 더 추천 받고자하면 y 아니면 n을 입력하는 부분
		while (true) { // 예외처리
	...
	protected int whichNumberOfTimeTable() { // 몇번쨰 시간표를 선택해서 추천받을지를 판별
		while (true) {
	...
	protected void ChoosingPrioritySubject(int newNumberOfSubjects) { // 내가원하는 과목들의 시간표를 만드는 메소드이다 이때 예외처리를 한다
	...
	protected String choicedTimeTables() { // 내가 GUI로 출력 할 시간표의 숫자를 정하기위한 메소드
		while (true) {
	...
	protected void ChoosingPriorityProfessor(int newNumberOfSubjects, LinkedList<AdjacencyTreeNode[]> aList) { // 예외처리
																												// 코드
	...
	protected LinkedList<LinkedList<AdjacencyTreeNode>> cutTheTimeTable(int cutLine,
	...
	protected int InputTheNumberOfSubjects() {
		return AppView.inputTheNumber();
	}

	protected void checkInputTheEmptyDay() { // 올바른 공강 날이 입력 됐는지 판별 해 준다
		while (true) {
	...
	protected void makeTheList() { // 필요한 정보로 list 형성
	...
	protected void makeTheTreeForTimeTable() { // list를 가지고 시간표 작성 해 주는 메소드
	...
	protected void lastRecomand() { // 교양 과목을 추천 받는 메소드
	...
	protected void makeTheGui(ArrayList<Object[][]> newObj) {
	...
	public void Run() {
		AppView.outputLine("A.S.A를 시작합니다.(*-*)/ ");
		AppView.outputLine("");
